...
const OneLanguageContent = ({ title,
    ...
    return <div className="flex flex-col max-w-[1290px] w-full break-words">
        ...
        <div className="mt-[30px] bg-[#EEF7FF] rounded-[10px] px-[40px] py-[30px] mb-[40px]">
            ...
            {
                policiesData[index].article.filter((data) => data.index && data[language]?.title ).map((data) => <div
        ...
        {
            policiesData[index].article.map((data) => <div className={data[language] ? data[language]?.noborder ? "mb-[15px]" : "mb-[39px]" : ""}
                ref={(el) => {
                    if (data.index ) {
